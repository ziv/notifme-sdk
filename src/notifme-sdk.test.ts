...
// $FlowIgnore
sdk.sender = {send: jest.fn()} as any;
...
test('mergeWithDefaultConfig should use given config if not empty.', () => {
  expect(sdk.mergeWithDefaultConfig(config as any)).toEqual(config);
});
...
test('mergeWithDefaultConfig should ignore config if useNotificationCatcher is true.', () => {
  expect(sdk.mergeWithDefaultConfig({...config, useNotificationCatcher: true} as any)).toEqual({
