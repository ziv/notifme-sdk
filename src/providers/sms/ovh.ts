...
export interface SmsOvhProviderConfig {
  appKey: string;
  appSecret: string;
  consumerKey: string;
  account: string;
...
export default class SmsOvhProvider {
  ...
  credentials: SmsOvhProviderConfig;

  constructor({appKey, appSecret, consumerKey, account, host}: SmsOvhProviderConfig) {
        ...
        : (messageClass === 1 ? 'phoneDisplay'
          : (messageClass === 2 ? 'sim'
            : (messageClass === 3 ? 'toolkit'
              : null))),
      // CR there is no type in request, but there is nature...
      // @ts-ignore
